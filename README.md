...
Автоматизация управления папками (folders) Telegram. Скрипт позволяет автоматически сортировать чаты и каналы по папкам, основываясь на шаблонах и исключениях, а также экспортировать структуру папок в YAML файл с поддержкой режима dry-run.
...
- ✅ Режим dry-run — проверка изменений без применения их к Telegram
...
# Настройки
settings:
    dry_run: false                          # Включить сухой запуск (без изменений в Telegram)
    export_enabled: true                    # Включить/выключить экспорт в YAML
...
|----------|-----|--------|---------|
| `dry_run` | boolean | `false` | Включить режим сухого запуска (без применения изменений к Telegram) |
| `export_enabled` | boolean | `false` | Включить экспорт структуры папок в YAML файл |
| `export_filename` | string | `folders_export.yaml` | Имя файла для экспорта |
...
Скрипт подключится к Telegram и применит все изменения согласно конфигурации.

### Режим dry-run (пробный запуск)

Для проверки изменений без применения их к Telegram установите в `config.yaml`:
...
В этом режиме:
- ✅ Все операции логируются и помечены `[DRY RUN]`
- ✅ Экспорт в YAML выполняется (с пометкой `dry_run: true`)
- ✅ **Никаких изменений не применяется к Telegram**
- ✅ Логи показывают, какие папки были бы созданы/обновлены
...
Dry-run режим идеален для:
- ✅ Проверки конфигурации перед применением
- ✅ Тестирования новых паттернов
- ✅ Планирования изменений
- ✅ Экспорта текущего состояния папок

### Пример вывода в режиме dry-run:

```
2025-10-23 14:37:00 - WARNING - ⚠️ DRY RUN MODE ENABLED - Никакие изменения не будут применены к Telegram
...
2025-10-23 14:37:02 - INFO - ✚ [DRY RUN] Would create folder "Работа" (ID=3, 7 chats)
2025-10-23 14:37:03 - INFO - ✎ [DRY RUN] Would update folder "Крипто" (12 chats)
2025-10-23 14:37:04 - INFO - − [DRY RUN] Would remove 2 chat(s) from "Dev" after moving to "Работа"
2025-10-23 14:37:05 - INFO - 📊 Статистика папок ПОСЛЕ обработки:
2025-10-23 14:37:05 - INFO -    📁 Работа: 7 групп/каналов
2025-10-23 14:37:05 - INFO -    📁 Крипто: 12 групп/каналов
2025-10-23 14:37:05 - INFO -    📁 Dev: 6 групп/каналов
2025-10-23 14:37:06 - INFO - 📤 [DRY RUN] Экспортировано 3 папок в файл "folders_export.yaml"
```

Заметьте, что все операции помечены `[DRY RUN]` и завершаются словом `Would` (было бы).
...
### Поля в экспорте:
- `export_date`: дата и время экспорта в ISO формате
- `dry_run`: был ли использован режим сухого запуска
- `folders`: словарь папок с их содержимым
  - `folder_id`: ID папки в Telegram
  - `chats_count`: количество чатов в папке
  - `chats`: список чатов с ID, названием и типом
...
include_patterns:
  ...
  - (dev|dev-team)   # Содержит "dev" или "dev-team"
```
...
- 📊 Статистика папок
- 📤 Экспорт данных
- [DRY RUN] Операции в режиме dry-run
...
1. **Создать конфигурацию** в `config.yaml`
2. **Включить dry-run режим**: `dry_run: true`
3. **Запустить скрипт**: `python3 -m tg_folder_manage`
4. **Проверить логи** — убедиться, что изменения соответствуют ожиданиям
5. **Выключить dry-run**: `dry_run: false`
6. **Запустить скрипт снова** — применить реальные изменения
...
4. Используйте `dry_run: true` для проверки без применения изменений
...
### Режим dry-run не работает
Убедитесь, что `dry_run: true` указан в `settings` в файле `config.yaml`, а не где-то ещё
